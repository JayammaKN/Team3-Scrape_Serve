...
// Whole-word match: "oil" matches "oil" but not "boiled".
...
async function setup() {
  ...
  const allergySheet = getSheetSafe(wb, CONFIG.allergiesSheetName); // Flat allergy list: Milk, Soy, Egg, Sesame, Peanuts, walnut, almond, hazelnut, pecan

  const allAllergens = excel.readColumn(
    allergySheet,
    "Allergies (Bonus points)",
  );
  ...
  const allergyOther = allAllergens.filter(
    (a) => !MILK.includes(norm(a)) && !NUTS.includes(norm(a)),
  );
...
export async function filterAndStore(recipe) {
  ...
  const ing = recipe.Ingredients; // ===== Allergy gate (runs first) =====
  // A. Milk?
  ...
  if (ingredientsMatchList(ing, lists.allergyNut)) {
    ...
    return;
  } // C. Other allergens (soy, egg, sesame)?
